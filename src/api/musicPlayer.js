...
//获取歌曲详情
export const getSongDetail = ids => request({ url: `/song/detail?ids=${ids}` });

//获取歌曲地址
// id歌曲id，level 歌曲的品质 默认标准"standard"，higher => 较高, exhigh=>极高, lossless=>无损, hires=>Hi-Res
export const getSongUrl  = (id,level='standard')=> request({url:`/song/url/v1?id=${31654343}&level=${level}`})


export const getLyric = id => request({ url: `/lyric?ids=${id}` });
